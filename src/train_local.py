"""Main Training Script for MAMBA-130M on Local Parquet Files"""
...
from .data_local import prepare_dataset_from_parquet, get_data_collator, compute_dataset_metrics
from .model import (
...
def train(
    ...
    logger.info("=" * 70)
    logger.info("Loading and preprocessing dataset from local parquet files...")
    logger.info("=" * 70)

    tokenized_dataset, tokenizer = prepare_dataset_from_parquet(
...
def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description="Train MAMBA-130M on Local Parquet Files")
    ...
    logger.info(f"Training data: {args.train_parquet}")
    logger.info(f"Validation data: {args.val_parquet}")
